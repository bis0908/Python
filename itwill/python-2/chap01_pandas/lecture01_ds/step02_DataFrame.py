...
# train index 
idx = choice(a=len(iris), 
             size = int(len(iris)*0.7), replace=False)
print(idx) # 행 번호 

train_set = iris.iloc[idx] # [idx, :]
...
# test index : list + for 
test_idx = [i  for i in range(len(iris))  if not i in idx ]
len(test_idx) # 45
test_idx
